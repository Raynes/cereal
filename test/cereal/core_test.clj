  ...
  (doseq [format [(reader/make) (proto/make Test$Foo)]]
    (testing "decode an encoded data structure"
      (let [val {:foo 1 :bar 2}]
        ...
        (is (= val (decode-stream format (ByteArrayInputStream. (encode format val)))))))
    ...
    (testing "append two simple encoded data structures"
      (let [data1 (encode format {:foo 1 :bar 2})
        ...
        (is (= {:foo 4 :bar 2 :baz 8}
               (decode-stream format (ByteArrayInputStream. (catbytes data1 data2)))))))
    ...
    (testing "merge maps when appending"
      (let [data1 (encode format {:num-map {1 "one" 3 "three"}})
        ...
        (is (= {:num-map {1 "one" 2 "dos" 3 "san" 6 "roku"}}
               (decode-stream format (ByteArrayInputStream. (catbytes data1 data2 data3)))))))

...
(deftest simple-codecs
  (doseq [codec [(json-codec :key-fn keyword) (edn-codec) (java-codec)]]
    (testing "decode an encoded data structure"
      (let [val {:foo 1 :bar 2}]
        (is (= val (decode codec (encode codec val)))))))

  (doseq [codec [(json-codec :validator map?)
                 (edn-codec :validator map?)
                 (java-codec :validator map?)]]
    (testing "validators"
...
(deftest appending
  (doseq [codec [(adjoining (edn-codec  :repeated true))
                 (adjoining (json-codec :key-fn keyword :repeated true))
